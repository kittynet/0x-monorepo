...
export async function ecSignOrderHashAsync(
    ...
    if (_.includes(validVParamValues, ecSignatureVRS.v)) {
        ...
        if (isValidVRSSignature) {
            const convertedSignatureHex = convertECSignatureToSignatureHex(
                ecSignatureVRS,
                messagePrefixOpts.prefixType,
            );
...
export function convertECSignatureToSignatureHex(
    ...
    messagePrefixType: MessagePrefixType,
): string {
    ...
    const signatureType = messagePrefixType === MessagePrefixType.Trezor ? SignatureType.Trezor : SignatureType.EthSign;
